...
def populateOldGame(t,wn,board):
    outFile=open("safeJumpH11.txt","r")
...
def win(board,player): #must add unable to move - hey, is there an easier way?
    blackCount=0
    redCount=0
    #print(board)
    if player==False:
...
def checkers():
    ...
    if currentPlayer=="Red":
        ...
        move=P2.getValidPlayerMove(board,currentPlayerTokens,rowInc,opposingPlayerTokens,currentPlayer,auto=True)
    while move != 'QUIT' and not win(board,False)[0]:
        if move == "Red":
    ...
    if move=='QUIT':
        ...
        print("Winner is player",win(board,False)[1]+"!!!")
